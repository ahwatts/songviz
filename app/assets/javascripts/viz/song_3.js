...
(function($) {
  ...
  var analyzers = [];
  var FREQ_BIN_COUNT = 1024;

  function initAudio() {
    // Handle the possibly-prefixed audio classes.
    try {
      AudioContext = window.AudioContext || window.webkitAudioContext;

      var i;
      var audio_elem = $("audio#song");
      var context = new AudioContext();
      var source = context.createMediaElementSource(audio_elem.get(0));
      ...
      alert("Your browser does not support the Web Audio API: " + e);
    }
  }

  /*
  function initControls() {
    $("a[data-song-elem][data-song-action=play]").click(function(event) {
      event.preventDefault();
      var elem = $(this);
      var canvas = $("canvas#viz");
      var song_elem_id = elem.attr("data-song-elem");
      var audio_elem = $(song_elem_id);
      canvas.data("stopVisualization", false);
      startAnimation();
    });

    $("a[data-song-elem][data-song-action=stop]").click(function(event) {
      event.preventDefault();
      var elem = $(this);
      var canvas = $("canvas#viz");
  ...
  */
  ...
  function createAmplitudeTexture() {
    ...
    return {
      "texture": texture,
      "update": function() {
        var i, eq_data = new Array(analyzers.length);
        var channel1 = 0;
        var channel2 = (channel1 + 1) % analyzers.length;

        for (i = 0; i < analyzers.length; ++i) {
          eq_data[i] = new Uint8Array(width);
          analyzers[i].getByteFrequencyData(eq_data[i]);
        }
